...
*	Creates a matrix or array filled with chi-squared random numbers.
*
* @param {Number|Number[]} [dims] - dimensions
* @param {Object} [opts] - function options
* @param {Number} [opts.k=1] - degrees of freedom
...
function random( dims, options ) {
	...
	if ( ndims === 1 ) {
		...
		if ( dt === 'generic' ) {
			return array( len, k, rand );
		}
		return typedarray( len, dt, k, rand );
	}
	// Multidimensional data structures...
	if ( dt !== 'generic' ) {
		if ( ndims === 2 ) {
			return matrix( dims, dt, k, rand );
		}
		// TODO: dstructs-ndarray support goes here. Until then, fall through to plain arrays...
	}
	return arrayarray( dims, k, rand );
} // end FUNCTION random()
